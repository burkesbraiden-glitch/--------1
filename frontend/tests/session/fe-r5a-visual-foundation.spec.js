...
describe('FE-R5A global visual foundation', () => {
  ...
  test('keeps AppTabbar navigation intact while consuming only equivalent bridge tokens', () => {
    ...
    expect(script).toContain("{ key: 'home', label: '首页', icon: '宫', path: '/pages/home/index' }")
    expect(script).toContain("{ key: 'plan', label: '计划', icon: '图', path: '/pages/plan/index' }")
    expect(script).toContain("{ key: 'tasks', label: '任务', icon: '勾', path: '/pages/tasks/index' }")
    expect(script).toContain("{ key: 'record', label: '记录', icon: '册', path: '/pages/record/index' }")
    expect(script).toContain("{ key: 'profile', label: '我的', icon: '我', path: '/pages/profile/index' }")
